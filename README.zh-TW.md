...
| 活動與提醒 | 資料夾位置、預設顏色、提醒開關與預設規則、三個通知管道、每日筆記區塊 |
...
`cli/otc_event.py` 讓 Claude Code 這類 agent 直接讀寫，會自己讀 vault 的每日筆記設定：

```bash
python3 cli/otc_event.py --vault <vault> list
python3 cli/otc_event.py --vault <vault> add --title "日本旅遊" --start 2026-10-24 --days 9 --dry-run
python3 cli/otc_event.py --vault <vault> --json add --title "帶電腦回家" --start 2026-08-29 --remind
python3 cli/otc_event.py --vault <vault> remove --id <uuid>
```

加 `--json` 會輸出單一 JSON 物件供程式解析。區間操作先用 `--dry-run` 看會碰哪些檔。
...
活動存在 `events.json`，位置由「設定 → 活動與提醒 → 資料夾」決定，預設 `open-taiwan-calendar`。
放在保管庫裡而不是插件設定裡，是為了讓它跟著同步走、能跟筆記一起進版控、也能用其他工具改。
插件會監看這個檔案，從外面改完不必重開 Obsidian。

### 三種新增方式

| 方式 | 說明 |
|------|------|
| 命令面板「新增活動／提醒」 | 打一句話，日期與天數自動讀出來 |
| 工具列的行事曆圖示 | 同上，預設日期是今天 |
| 日期格右鍵 →「在這天新增活動」 | 預設日期是你點的那一天 |
| 筆記裡選一行 → 右鍵 →「新增提醒」／「新增活動」 | 選到的文字直接當成句子解析；選「新增提醒」會預先打開提醒開關 |
| 設定頁「你的活動」→ 新增活動 | 同一個視窗，也可在此編輯或刪除既有項目 |

日期預設值：有開著每日筆記時用那一天，沒有就用今天；從日期格右鍵進來則用你點的那一天。
「開始」「結束」右邊各有一顆行事曆按鈕，可開系統日期選擇器。
